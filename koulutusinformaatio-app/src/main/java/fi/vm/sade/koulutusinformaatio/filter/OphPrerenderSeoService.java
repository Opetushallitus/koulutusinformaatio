...
/**
 * Copied from https://github.com/greengerong/prerender-java/blob/01d0511e859458078379b258972a066be58483c6/src/main/java/com/github/greengerong/PrerenderSeoService.java
 * which is unfortunately not very customisable.
 *
 * Our change is to allow using asynchronous Servlet filter processing.
 */
public class OphPrerenderSeoService {
    ...
    private final ExecutorService prerenderExecutorService;
    private PreRenderEventHandler preRenderEventHandler;

    public OphPrerenderSeoService(Map<String, String> config, ExecutorService prerenderExecutorService) {
        this.prerenderConfig = new PrerenderConfig(config);
        this.prerenderExecutorService = prerenderExecutorService;
    ...
    private boolean handlePrerender(final HttpServletRequest servletRequest, final HttpServletResponse servletResponse)
        ...
        if (shouldShowPrerenderedPage(servletRequest)) {
            ...
            prerenderExecutorService.execute(new Runnable() {
                @Override
                public void run() {
                    try {
                        ...
                        if (!(beforeRender(request, response) || proxyPrerenderedPageResponse(request, response))) {
                            log.error("Looks like prerendering did not work.");
                        }
                    } catch (Exception e) {
                        log.error("Got exception on prerender call", e);
